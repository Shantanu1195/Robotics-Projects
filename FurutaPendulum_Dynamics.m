...
DH=[[0;q(1);0;-pi/2] [2;q(2);0;0]]; %define DH parameters
n=2;%number of joints
m=[1 1]; %define mass constants
I(:,:,1)=[1 0 0;0 1 0;0 0 1];I(:,:,2)=[1 0 0;0 0 0;0 0 1];%define inertia tensor
r_C=[[0;0;0] [0;0.5;0]];%define com coordinates of each link
kv=[0.5;0.5];%define viscous friction constants
g0=[0;0;-9.8];%define gravity vector

%%%define joint and link geometric parameters for drawing
Joint{1}.length=0.5;
Joint{1}.radius=0.1;
...
Link{2}.radius=0.1;
Link{2}.midpoint=[0;0.5;0];%mid point of link 2 represented in frame {2}
Link{2}.length=1;%lenght of link 2
Link{2}.R=rot([1;0;0],-pi/2);% rotation matrix of the link cylinder 2 wrt frame {2}
