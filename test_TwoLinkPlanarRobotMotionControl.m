...
while t<10
    ...
    qd=[pi/2-pi/4*cos(2*pi/2*t);pi/4*sin(2*pi/2*t)];
    qd_dot=[pi/4*2*pi/2*sin(2*pi/2*t);pi/4*2*pi/2*cos(2*pi/2*t)];
    qd_ddot=[pi/4*((2*pi/2)^2)*cos(2*pi/2*t);-pi/4*((2*pi/2)^2)*sin(2*pi/2*t)];
    e=q-qd;
    e_dot=q_dot-qd_dot;
    %decentralized control without feedforward
%     Kp=25;Kd=5;
%     tau(1,1)=-B(1,1)*Kp*e(1)-B(1,1)*Kd*q_dot(1)+kv(1)*q_dot(1);
%     tau(2,1)=-B(2,2)*Kp*e(2)-B(2,2)*Kd*q_dot(2)+kv(2)*q_dot(2);
    %decentralized control with feedforward for trajectory tracking    
%     Kp=25;Kd=5;
%     tau(1,1)=-B(1,1)*Kp*e(1)-B(1,1)*Kd*e_dot(1)+B(1,1)*qd_ddot(1)+kv(1)*q_dot(1);
%     tau(2,1)=-B(2,2)*Kp*e(2)-B(2,2)*Kd*e_dot(2)+B(2,2)*qd_ddot(2)+kv(2)*q_dot(2);
    %decentralized control with computed torque compensation
%     Kp=25;Kd=5;
%     tau(1,1)=-B(1,1)*Kp*e(1)-B(1,1)*Kd*e_dot(1)+B(1,1)*qd_ddot(1)+kv(1)*q_dot(1);
%     tau(2,1)=-B(2,2)*Kp*e(2)-B(2,2)*Kd*e_dot(2)+B(2,2)*qd_ddot(2)+kv(2)*q_dot(2);
%     DH_d=DH;
%     DH_d(2,1)=qd(1);DH_d(2,2)=qd(2);
%     for i=1:n
%         q_ddot_temp=[0;0];
%         q_ddot_temp(i)=1;
%         B_d(:,i) = Newton_Euler_Iteration(DH_d, m, I, r_C, Jtype, [0;0], q_ddot_temp, [0;0;0]);
%     end
%     B_tilded=-B_d;
%     B_tilded(1,1)=0;B_tilded(2,2)=0;
%     Coriolis_Gravity_d=Newton_Euler_Iteration(DH_d, m, I, r_C, Jtype, qd_dot, [0;0], g0);
%     tau=tau-B_tilded*qd_ddot+Coriolis_Gravity_d;
    
    % centralized control
    Kp=25.*eye(2);Kd=5.*eye(2);
    tau=-B*Kp*e-B*Kd*e_dot+Coriolis_Gravity+kv.*q_dot+B*qd_ddot;
...
figure(2)
plot(t_stack,e_stack(1,:),'r');
hold on
plot(t_stack,e_stack(2,:),'b');
xlim([0 10]);
% ylim([-0.8 0.3]);
legend('e_1','e_2');
xlabel('time (sec)');
